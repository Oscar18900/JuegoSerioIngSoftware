...
// Lee el high score de un proyecto desde localStorage
...
// Exportada para que la llames desde CascadaP1/P2/P3 al terminar el juego
export function saveHighScore(proyectoId, puntos) {
  try {
    const actual = getHighScore(proyectoId);
    if (actual === null || puntos > actual) {
      localStorage.setItem(`hs_${proyectoId}`, puntos.toString());
    }
  } catch {}
}

// Rango según puntuación
function getRango(puntos) {
  if (puntos >= 800) return { label: 'ORO',    color: 'text-yellow-400', symbol: '🥇' };
